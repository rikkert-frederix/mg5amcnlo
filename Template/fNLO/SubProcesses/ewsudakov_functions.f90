module ewsudakov_functions_module
  use process_dimensions, only: nexternal, nsplitorders, qcd_pos, &
       qed_pos, amp_split_size, amp_split_size_born, born_orders, &
  ...
  double precision :: g = 0d0
  complex(kind=kind(0d0)) :: gal(2) = (0d0, 0d0)
  double precision :: mdl_mh = 0d0, mdl_mt = 0d0
  double precision :: mdl_mw = 0d0, mdl_mz = 0d0
  double precision :: mdl_ee = 0d0, aewm1 = 0d0
  complex(kind=kind(0d0)) :: mdl_gf = (0d0, 0d0)
  double precision :: qes2 = 0d0, scale = 0d0
  integer :: sud_mod = 2, deb_settozero = 0
  logical :: fav4 = .false., s_to_rij = .true.
  logical :: rij_ge_mw = .true., printinewsdkf = .false.
  logical :: has_lo1 = .false., has_lo2 = .false.
  logical :: state_initialized = .false.
  logical :: defaults_initialized = .false.
  logical :: has_lo2_cache_valid = .false.
  logical :: has_lo2_cache = .true.
  integer, allocatable :: lo2_orders_cache(:)

  public :: initialize_ewsudakov_state
  public :: initialize_ewsudakov_defaults
  public :: has_lo2_impl, get_lo2_orders
  public :: get_imlog, get_lsc_diag, get_lsc_nondiag
  public :: get_ssc_c, get_ssc_n_diag
  public :: get_ssc_n_nondiag_1, get_ssc_n_nondiag_2
  public :: get_xxc_diag, get_xxc_nondiag
  public :: bigl, smalll, log_a_over_b_sing
  public :: smalll_a_over_b_sing, bigl_a_over_b_sing
  public :: biglem, smalllem, dzemaa_logs
  public :: sdk_chargesq, sdk_charge, sdk_tpm
  public :: sdk_t3_diag, sdk_yo2_diag, sdk_iz_diag
  public :: sdk_iz_nondiag, sdk_ia_diag, sdk_iz2_diag
  public :: sdk_cew_diag, sdk_cew_nondiag
  public :: sdk_betaew_diag, sdk_betaew_nondiag
  public :: sdk_get_invariants, sdk_test_functions
  public :: get_isopart_mass_from_id
  public :: get_par_ren_alphamz, get_par_ren_gmu, get_qcd_lo2

  interface
    double precision function get_mass_from_id(pdg)
      implicit none
...
contains
  ...
  subroutine initialize_ewsudakov_defaults(sud_mod_out, &
       sud_filter_hel_out, sud_mc_hel_out, fav4_out, s_to_rij_out, &
       cs_run_out, rij_ge_mw_out)
    implicit none
    integer, intent(inout) :: sud_mod_out
    logical, intent(inout) :: sud_filter_hel_out, sud_mc_hel_out
    logical, intent(inout) :: fav4_out, s_to_rij_out, cs_run_out
    logical, intent(inout) :: rij_ge_mw_out

    if (defaults_initialized) return

    sud_mod_out = 2
    sud_filter_hel_out = .true.
    sud_mc_hel_out = .true.
    fav4_out = .false.
    s_to_rij_out = .true.
    cs_run_out = .false.
  ...
  subroutine initialize_ewsudakov_state(g_in, gal_in, &
       mdl_mh_in, mdl_mt_in, mdl_mw_in, mdl_mz_in, mdl_ee_in, &
       aewm1_in, mdl_gf_in, qes2_in, scale_in, sud_mod_in, fav4_in, &
       s_to_rij_in, rij_ge_mw_in, printinewsdkf_in, deb_settozero_in, &
       has_lo1_in, has_lo2_in)
    implicit none
    double precision, intent(in) :: g_in, mdl_mh_in, mdl_mt_in
    double precision, intent(in) :: mdl_mw_in, mdl_mz_in, mdl_ee_in
    double precision, intent(in) :: aewm1_in, qes2_in, scale_in
    complex(kind=kind(0d0)), intent(in) :: gal_in(2), mdl_gf_in
    integer, intent(in) :: sud_mod_in, deb_settozero_in
    logical, intent(in) :: fav4_in, s_to_rij_in, rij_ge_mw_in
    logical, intent(in) :: printinewsdkf_in, has_lo1_in, has_lo2_in

    call validate_process_dimensions()
    if (.not. allocated(lo2_orders_cache)) then
      allocate(lo2_orders_cache(nsplitorders))
    else if (size(lo2_orders_cache) /= nsplitorders) then
      write (*, *) 'EW Sudakov state has inconsistent split-order size'
      stop 1
    end if
    ...
    sud_mod = sud_mod_in
    fav4 = fav4_in
    s_to_rij = s_to_rij_in
    rij_ge_mw = rij_ge_mw_in
    printinewsdkf = printinewsdkf_in
    deb_settozero = deb_settozero_in
    has_lo1 = has_lo1_in
    has_lo2 = has_lo2_in
    state_initialized = .true.
  end subroutine initialize_ewsudakov_state

  subroutine require_ewsudakov_state()
    implicit none
    if (.not. state_initialized) then
      write (*, *) 'EW Sudakov module used before initialization'
      stop 1
    end if
  end subroutine require_ewsudakov_state

  logical function real_is_zero(value)
    implicit none
    double precision, intent(in) :: value

    real_is_zero = value <= 0d0 .and. value >= 0d0
  end function real_is_zero

  logical function complex_is_zero(value)
    implicit none
    complex(kind=kind(0d0)), intent(in) :: value

    complex_is_zero = real_is_zero(dble(value)) .and. &
         real_is_zero(aimag(value))
  end function complex_is_zero

  subroutine acknowledge_integer(value)
    implicit none
    integer, intent(in) :: value

    if (value > huge(value)) stop 1
  end subroutine acknowledge_integer

  subroutine acknowledge_integer_array(values)
    implicit none
    integer, intent(in) :: values(:)

    if (size(values) < 0) stop 1
  end subroutine acknowledge_integer_array

logical function has_lo2_impl()
implicit none
integer, allocatable :: orders_check(:)
integer :: i

call require_ewsudakov_state()
if (has_lo2_cache_valid) then
  has_lo2_impl = has_lo2_cache
  return
end if

call get_lo2_orders(lo2_orders_cache)
allocate(orders_check(nsplitorders))
has_lo2_cache = .true.
do i = 1, amp_split_size_born
  has_lo2_cache = .true.
  call amp_split_pos_to_orders(i, orders_check)
  has_lo2_cache = all(orders_check == lo2_orders_cache)
  if (has_lo2_cache) exit
end do
deallocate(orders_check)
has_lo2_cache_valid = .true.
has_lo2_impl = has_lo2_cache

return
end function has_lo2_impl
...
return
end subroutine get_lo2_orders



!! MZ declare all functions as complex(kind=kind(0d0)), since some (few)
!  terms can be imaginary

complex(kind=kind(0d0)) function get_imlog(s)
implicit none
double precision s
...
get_imlog = cmplx(0d0,0d0,kind=kind(0d0))
if (s.gt.0d0.and..not.force_imlog_to_zero) &
  get_imlog = cmplx(0d0,-1d0*pi,kind=kind(0d0))

return
end function get_imlog


complex(kind=kind(0d0)) function get_lsc_diag(pdglist, hels, iflist, invariants)
implicit none
...
get_lsc_diag = get_lsc_diag - 0.5d0 * &
  ...
  & )
lzfordebug = dble(lzfordebug - 0.5d0 * ( &
  & - 2d0*lzow*sdk_iz2_diag(pdglist(i),hels(i),iflist(i))* &
  & smallL(invariants(1,2))))
enddo
...
return
end function get_lsc_diag


complex(kind=kind(0d0)) function get_lsc_nondiag(pdglist, hels, iflist, &
...
call acknowledge_integer_array(pdglist)
call acknowledge_integer_array(iflist)
...
return
end function get_lsc_nondiag


complex(kind=kind(0d0)) function get_ssc_c(ileg1, ileg2, pdglist, pdgp1, pdgp2, hels, iflist, invariants)
implicit none
...
complex(kind=kind(0d0)) imlog



complex(kind=kind(0d0)) smallL_rij_over_s, bigL_rij_over_s
...
if (printinewsdkf) WRITE (72,*) hels, ileg1, ileg2, pdgp1, pdgp2, &
  & dble(sdk_tpm(pdglist(ileg1), hels(ileg1), iflist(ileg1), pdgp1)* &
  & cmplx(1d0,-1000d0,kind=kind(0d0))), &
  & dble(sdk_tpm(pdglist(ileg2), hels(ileg2), iflist(ileg2), pdgp2)* &
  & cmplx(1d0,-1000d0,kind=kind(0d0)))

if(deb_settozero.ne.0.and.deb_settozero.ne.1.and.deb_settozero.ne.111) get_ssc_c = 0d0

return
end function get_ssc_c


complex(kind=kind(0d0)) function get_ssc_n_diag(pdglist, hels, iflist, invariants)
implicit none
integer pdglist(nexternal-1), hels(nexternal-1), iflist(nexternal-1)
double precision invariants(nexternal-1, nexternal-1)
double precision lzow
...
!      2d0/3d0*smallLem(0d0) comes from l(MW2,0d0) in the formulas
...
return
end function get_ssc_n_diag


complex(kind=kind(0d0)) function get_ssc_n_nondiag_1(pdglist, hels, iflist, &
  & invariants, ileg, pdg_old, pdg_new)
implicit none
integer pdglist(nexternal-1), hels(nexternal-1), iflist(nexternal-1)
double precision invariants(nexternal-1, nexternal-1)
...
! not work for H or Chi0 in initial state
...
if (printinewsdkf) WRITE (72,*) hels, ileg, i, pdg_new,pdglist(i), &
  & dble(sdk_iz_nondiag(pdg_new,hels(ileg),iflist(ileg))* &
  & cmplx(1d0,-1000d0,kind=kind(0d0))), &
  & dble(sdk_iz_diag(pdglist(i),hels(i),iflist(i))* &
  & cmplx(1d0,-1000d0,kind=kind(0d0)))
...
return
end function get_ssc_n_nondiag_1


complex(kind=kind(0d0)) function get_ssc_n_nondiag_2(pdglist, hels, iflist, &
...
if (printinewsdkf) WRITE (72,*) hels, ileg1, ileg2, pdg_new1, pdg_new2, &
  & dble(sdk_iz_nondiag(pdg_new1,hels(ileg1),iflist(ileg1))* &
  & cmplx(1d0,-1000d0,kind=kind(0d0))), &
  & dble(sdk_iz_nondiag(pdg_new2,hels(ileg2),iflist(ileg2))* &
  & cmplx(1d0,-1000d0,kind=kind(0d0)))
...
return
end function get_ssc_n_nondiag_2


complex(kind=kind(0d0)) function get_xxc_diag(pdglist, hels, iflist, invariants)
implicit none
integer pdglist(nexternal-1), hels(nexternal-1), iflist(nexternal-1)
double precision invariants(nexternal-1, nexternal-1)
integer i
...
! uncomment if uncomment the full block down
!        tmp= get_xxc_diag
enddo
...
return
end function get_xxc_diag


complex(kind=kind(0d0)) function get_xxc_nondiag(pdglist, hels, iflist, &
  & invariants, ileg, pdg_old, pdg_new)
implicit none
integer pdglist(nexternal-1), hels(nexternal-1), iflist(nexternal-1)
double precision invariants(nexternal-1, nexternal-1)
...
return
end function get_xxc_nondiag


complex(kind=kind(0d0)) function bigL(s)
implicit none
double precision s
double precision pi
parameter (pi=3.14159265358979323846d0)
...
complex(kind=kind(0d0)) function smallL(s)
implicit none
double precision s
double precision pi
parameter (pi=3.14159265358979323846d0)
...
complex(kind=kind(0d0)) function log_a_over_b_sing(a,b)
implicit none
double precision a,b,aok,bok
if ((dabs(a).lt.1d0.and..not.real_is_zero(dabs(a))).or. &
  & (dabs(b).lt.1d0.and..not.real_is_zero(dabs(b)))) then
...
aok=a
bok=b
if (real_is_zero(a)) aok= QES2
if (real_is_zero(b)) bok= QES2
...
return
end function log_a_over_b_sing

complex(kind=kind(0d0)) function smallL_a_over_b_sing(a,b)
implicit none
...
return
end function smallL_a_over_b_sing


complex(kind=kind(0d0)) function bigL_a_over_b_sing(a,b)
implicit none
double precision a,b
double precision pi
parameter (pi=3.14159265358979323846d0)
...
!      print*, "in bigL a over b", " a=", a, " b=", b

return
end function bigL_a_over_b_sing


complex(kind=kind(0d0)) function bigLem(s,m2k)
implicit none
...
complex(kind=kind(0d0)) function smallLem(m2k)
implicit none
...
end function smallLem


complex(kind=kind(0d0)) function dZemAA_logs()
implicit none
if (sud_mod.eq.0.or.sud_mod.eq.1) then
dZemAA_logs= 0d0
return
...
return
end function dZemAA_logs



complex(kind=kind(0d0)) function sdk_chargesq(pdg, hel, ifsign)
implicit none
...
return
end function sdk_chargesq


complex(kind=kind(0d0)) function sdk_charge(pdg, hel, ifsign)
implicit none
integer pdg, hel, ifsign

integer s_pdg

call acknowledge_integer(hel)
...
! charged goldstones / W boson
if (s_pdg.eq.251.or.s_pdg.eq.24) sdk_charge = 1d0
if (s_pdg.eq.-251.or.s_pdg.eq.-24) sdk_charge = -1d0

if (.not.FAV4) then
sdk_charge = -1 * sdk_charge
if (abs(s_pdg).eq.251) sdk_charge = sdk_charge * &
  cmplx(0d0,1d0,kind=kind(0d0))
endif

return
end function sdk_charge


complex(kind=kind(0d0)) function sdk_tpm(pdg, hel, ifsign, pdgp)
implicit none
...
! the product of pdg code * helicity
! Hel=+1/-1 -> R/L. Note that for transverse polarisations it does not depend
! on "ifsign", since switching from final to initial changes both the pdg and the helicity

!!!!!						ATTENTION						      !!!!
!!!!! following the notation of Denner and Pozzorini, the prime index pdgp is the first and pdg is the second !!!!
!!!!! It is not working for logitudinally polarised Z in the initial state (ifsign=-1), both as pdg or pdgp   !!!!
...
! and return
if (.not.complex_is_zero(sdk_tpm)) then
...
! goldstones, they behave like left handed leptons (charged) or neutrinos (neutrals)
if (abs(s_pdg).eq.251.and.pdgp.eq.25) sdk_tpm = sign(1d0,dble(s_pdg))
if (abs(s_pdg).eq.251.and.pdgp.eq.250) &
  sdk_tpm = cmplx(0d0,-1d0,kind=kind(0d0))

if (.not.complex_is_zero(sdk_tpm)) then
sdk_tpm = sdk_tpm / ( 2d0 * dsqrt(sw2))

if (.not.FAV4) sdk_tpm = sdk_tpm * &
  cmplx(0d0,1d0,kind=kind(0d0))
...
! following last .and. conditions are not strictly necessary
if (abs(s_pdg).eq.250.and.abs(pdgp*ifsign).eq.251) &
  sdk_tpm = cmplx(0d0,1d0,kind=kind(0d0))
...
return
end function sdk_tpm



complex(kind=kind(0d0)) function sdk_t3_diag(pdg, hel, ifsign)
implicit none
integer pdg, hel, ifsign
integer s_pdg
double precision sw2, cw2
...
! goldstones, they behave like left handed leptons (charged); neutrals
! mix
...
if (abs(s_pdg).eq.251) sdk_t3_diag = sdk_t3_diag * &
  cmplx(0d0,1d0,kind=kind(0d0))
endif

return
end function sdk_t3_diag



complex(kind=kind(0d0)) function sdk_yo2_diag(pdg, hel, ifsign)
implicit none
integer pdg, hel, ifsign
integer s_pdg
double precision sw2, cw2
...
! right handed lepton / left handed antilepton
...
! right handed up quark / left handed antiup quark
...
! right handed down quark / left handed antidown quark
...
! goldstones, they behave like left handed leptons (charged); neutrals mix
...
if (abs(s_pdg).eq.251) sdk_yo2_diag = sdk_yo2_diag * &
  cmplx(0d0,1d0,kind=kind(0d0))
endif

return
end function sdk_yo2_diag


complex(kind=kind(0d0)) function sdk_iz_diag(pdg, hel, ifsign)
implicit none
integer pdg, hel, ifsign
double precision sw2, cw2
...
if (abs(pdg).eq.251) sdk_iz_diag = sdk_iz_diag / &
  cmplx(0d0,1d0,kind=kind(0d0))

endif

return
end function sdk_iz_diag


complex(kind=kind(0d0)) function sdk_iz_nondiag(pdg, hel, ifsign)
implicit none
integer pdg, hel, ifsign
double precision sw2, cw2
...
! pdg (new) = 25
if (pdg.eq.25) sdk_iz_nondiag = ifsign * &
  cmplx(0d0,-1d0,kind=kind(0d0)) / 2d0
if (pdg.eq.250) sdk_iz_nondiag = ifsign * &
  cmplx(0d0,1d0,kind=kind(0d0)) / 2d0
...
return
end function sdk_iz_nondiag


complex(kind=kind(0d0)) function sdk_ia_diag(pdg, hel, ifsign)
implicit none
integer pdg, hel, ifsign
...
return
end function sdk_ia_diag



complex(kind=kind(0d0)) function sdk_iz2_diag(pdg, hel, ifsign)
implicit none
integer pdg, hel, ifsign
integer s_pdg
double precision sw2, cw2
...
return
end function sdk_iz2_diag




complex(kind=kind(0d0)) function sdk_cew_diag(pdg, hel, ifsign)
implicit none
integer pdg, hel, ifsign
integer s_pdg
double precision sw2, cw2
...
! goldstones and Higgs, they behave like left handed leptons (charged) or neutrinos (neutrals)
...
return
end function sdk_cew_diag


complex(kind=kind(0d0)) function sdk_cew_nondiag()
implicit none
! returns the gamma/z mixing of sdk_cew
double precision sw2, cw2
...
return
end function sdk_cew_nondiag


complex(kind=kind(0d0)) function sdk_betaew_diag(pdg)
implicit none
integer pdg
double precision sw2, cw2
...
return
end function sdk_betaew_diag


complex(kind=kind(0d0)) function sdk_betaew_nondiag()
implicit none
double precision sw2, cw2
...
return
end function sdk_betaew_nondiag
...
do i = 1, nexternal-1
do j = i, nexternal-1
invariants(i,j) = sumdot(p(:,i),p(:,j),dble(iflist(i)*iflist(j)))
...
return
end subroutine sdk_get_invariants
...
complex(kind=kind(0d0)) q, t3, yo2, iz2
...
return
end subroutine sdk_test_functions
...
return
end function get_isopart_mass_from_id



subroutine get_par_ren_alphamz(invariants, amp_split_ewsud_der, &
  & amp_split_ewsud_der2, amp_split_ewsud)
implicit none
complex(kind=kind(0d0)), intent(in) :: amp_split_ewsud_der(:, :)
complex(kind=kind(0d0)), intent(in) :: amp_split_ewsud_der2(:, :)
complex(kind=kind(0d0)), intent(out) :: amp_split_ewsud(:)
double precision invariants(nexternal-1, nexternal-1)
!     ipara = 1->AEWm1; 2->MZ; 3->MW; 4->MT/YMT; 5->MH
complex(kind=kind(0d0)) ls, ls_o_mt
complex(kind=kind(0d0)) dalpha, dmw2, dmz2, dmt, dmh2, dt, dheff_o_heff
complex(kind=kind(0d0)) dmt_QCD
...
!      print*,"from dmt_QCD", amp_split_ewsud_der2(:,4) * dmt_QCD
endif
endif
...
return
end subroutine get_par_ren_alphamz



subroutine get_par_ren_gmu(invariants, amp_split_ewsud_der, &
  & amp_split_ewsud_der2, amp_split_ewsud)
implicit none
complex(kind=kind(0d0)), intent(in) :: amp_split_ewsud_der(:, :)
complex(kind=kind(0d0)), intent(in) :: amp_split_ewsud_der2(:, :)
complex(kind=kind(0d0)), intent(out) :: amp_split_ewsud(:)
double precision invariants(nexternal-1, nexternal-1)
!     ipara = 1->GF; 2->MZ; 3->MW; 4->MT/YMT; 5->MH
complex(kind=kind(0d0)) ls, ls_o_mt
complex(kind=kind(0d0)) dalpha, dmw2, dmz2, dmt, dmh2, dt, dheff_o_heff
complex(kind=kind(0d0)) dmt_QCD, dGmu, dGmudalpha, dGmudmz2, dGmudmw2
double precision pi, cw2, sw2, Qt
parameter (pi=3.14159265358979323846d0)
...
return
end subroutine get_par_ren_gmu


complex(kind=kind(0d0)) function GET_QCD_LO2(pdglist, hels, iflist, invariants,iamp)
implicit none
integer pdglist(nexternal-1), hels(nexternal-1), iflist(nexternal-1)
double precision invariants(nexternal-1, nexternal-1)
!      complex(kind=kind(0d0)) bigL, smallL, smallLem
!      external smallLem
...
!      complex(kind=kind(0d0)) smallL_a_over_b_sing, bigL_a_over_b_sing
...
call acknowledge_integer_array(hels)
call acknowledge_integer_array(iflist)
get_qcd_lo2 = cmplx(0d0,0d0,kind=kind(0d0))
...
! Preserve the historical CMPLX conversion, which rounded through the
! default real kind before assignment to the complex result.
get_qcd_lo2 = cmplx(real(logfromLOip1),0.0)
...
return
end function GET_QCD_LO2


end module ewsudakov_functions_module
