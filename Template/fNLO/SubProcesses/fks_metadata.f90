module fks_metadata
  use process_dimensions, only: nexternal, fks_configs, &
  ...
  public :: particle_type_d, pdg_type_d
  public :: need_color_links_d

contains

  subroutine initialize_fks_metadata(fks_i_in, fks_j_in, &
       fks_j_from_i_in, particle_type_in, pdg_type_in, &
       need_color_links_in)
    ...
    call validate_input_shapes(fks_i_in, fks_j_in, fks_j_from_i_in, &
         particle_type_in, pdg_type_in, need_color_links_in)
    call validate_input_values(fks_i_in, fks_j_in, fks_j_from_i_in)

    if (metadata_initialized) then
      if (.not. same_metadata(fks_i_in, fks_j_in, fks_j_from_i_in, &
           particle_type_in, pdg_type_in, need_color_links_in)) then
  ...
  subroutine validate_input_shapes(fks_i_in, fks_j_in, &
       fks_j_from_i_in, particle_type_in, pdg_type_in, &
       need_color_links_in)
    integer, intent(in) :: fks_i_in(:), fks_j_in(:)
    integer, intent(in) :: fks_j_from_i_in(:, :, 0:)
  ...
  subroutine validate_input_values(fks_i_in, fks_j_in, fks_j_from_i_in)
    integer, intent(in) :: fks_i_in(:), fks_j_in(:)
    integer, intent(in) :: fks_j_from_i_in(:, :, 0:)
  ...
  logical function same_metadata(fks_i_in, fks_j_in, &
       fks_j_from_i_in, particle_type_in, pdg_type_in, &
       need_color_links_in)
    integer, intent(in) :: fks_i_in(:), fks_j_in(:)
    integer, intent(in) :: fks_j_from_i_in(:, :, 0:)
