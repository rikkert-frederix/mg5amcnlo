c Analysis ABI for the jet-based trilepton ttW product study.

      subroutine analysis_begin(nwgt,weights_info)
      use analysis_hwu_pp_ttxw_product_module,
     &     only: module_begin => analysis_begin
      implicit none
      integer nwgt
      character(len=*) weights_info(*)
      call module_begin(nwgt,weights_info)
      end

      subroutine analysis_end()
      use analysis_hwu_pp_ttxw_product_module,
     &     only: module_end => analysis_end
      implicit none
      call module_end()
      end
