module analysis_hwu_pp_hjj_module
  ...
  double precision :: vetomin = 0d0
  double precision :: vetomax = 0d0
  integer :: nbinveto = 0
  logical :: veto_state_initialized = .false.
...
!
  call initialize_veto_state(nwgt)
...
!
  ...
  veto_weights = wgts(1:size(veto_weights))/xsecup2
  call HwU_fill(l+49, (vetomax-vetomin)* dble(k) &
  & /dble(nbinveto)*0.99, veto_weights)
  endif
  enddo
...
!
  ...
  subroutine initialize_veto_state(nwgt)
    implicit none
    integer, intent(in), optional :: nwgt

    if (.not. veto_state_initialized) then
      vetomin = 0d0
      vetomax = 100d0
      nbinveto = 50
      veto_state_initialized = .true.
    end if

    if (.not. present(nwgt)) return
    if (allocated(veto_weights)) then
      if (size(veto_weights) == nwgt) return
      deallocate(veto_weights)
    end if
    allocate(veto_weights(nwgt))
    veto_weights = 0d0
  end subroutine initialize_veto_state
