...
!
! Example analysis for "p p > t j [QCD]" process.
! Example analysis for "p p > t j $$ w+ w- [QCD]" process.
! Example analysis for "p p > w+ > t j  [QCD]" process.
!
...
! fill the histograms
  ...
  function getrapidity(p)
    implicit none
    double precision getrapidity, en, pl, tiny, xplus, xminus, y, p(0:3)
...
!
  ...
  function getpseudorap(p)
    implicit none
    double precision getpseudorap, en, ptx, pty, pl, tiny, pt, eta, th, p(0:3)
    parameter(tiny=1.d-5)
!
    en = p(0)
    ptx = p(1)
  ...
  function getpt(p)
    implicit none
    double precision getpt, p(0:3)
    getpt = dsqrt(p(1)**2 + p(2)**2)
    return
  end function getpt

end module analysis_hwu_pp_tj_module
