module analysis_hwu_general_sdk_module
  use process_dimensions, only: nexternal, nincoming
  use run_state, only: jetalgo, jetradius, ptj, etaj, rphreco, &
       etaphreco, lepphreco, quarkphreco
...
!     See also line 376 in this file
  ...
  do i=1,4
! if the splitorders are histogrammed one by one, uncomment what the
! following line and comment the one above.
! Note that the sudakov corrections will not be done order by order
!      do i=1,16
...
!
!         if (i.eq. 5.and.orders_tag_plot.ne.4) cycle
!         if (i.eq. 6.and.orders_tag_plot.ne.202) cycle
!         if (i.eq. 7.and.orders_tag_plot.ne.400) cycle
!         if (i.eq. 8.and.orders_tag_plot.ne.6) cycle
!         if (i.eq. 9.and.orders_tag_plot.ne.204) cycle
!         if (i.eq.10.and.orders_tag_plot.ne.402) cycle
!         if (i.eq.11.and.orders_tag_plot.ne.600) cycle
!         if (i.eq.12.and.orders_tag_plot.ne.8) cycle
!         if (i.eq.13.and.orders_tag_plot.ne.206) cycle
!         if (i.eq.14.and.orders_tag_plot.ne.404) cycle
!         if (i.eq.15.and.orders_tag_plot.ne.602) cycle
!         if (i.eq.16.and.orders_tag_plot.ne.800) cycle
...
end module analysis_hwu_general_sdk_module
